...
<body class="bg-white">
    ...
    <main class="container mx-auto py-8 px-6">
        ...
        <td class="py-2 px-4">
            <form action="{{ route('admin.updateStatus', $order->id) }}" method="POST" class="flex items-center">
                @csrf
                @method('PATCH')
                <select name="status" class="border border-gray-300 rounded py-1 px-2">
                    <option value="pending" {{ $order->status === 'pending' ? 'selected' : '' }}>Pending</option>
                    <option value="approved" {{ $order->status === 'approved' ? 'selected' : '' }}>Approved</option>
                    <option value="beingDelivered" {{ $order->status === 'beingDelivered' ? 'selected' : '' }}>Being Delivered</option>
                    <option value="completed" {{ $order->status === 'completed' ? 'selected' : '' }}>Completed</option>
                </select>
                <button type="submit" class="ml-2 px-2 py-1 bg-green-500 text-white rounded">Update</button>
            </form>
        </td>        
            ...
            <table class="min-w-full bg-white shadow-md rounded-lg">
                ...
                <tbody>
                    <tr class="border-b border-green-500">
                        ...
                        <td class="py-2 px-4">{{ Str::ucfirst($order->status) }}</td>
                    </tr>
                </tbody>
            </table>
        </div>
